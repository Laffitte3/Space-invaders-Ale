...
while running:
    ...
    #Pasar esto al for
    #alien_x_random[i] -= alien_x_change[i]
    ...
    for i in range(nuemro_de_enemigos):
        ...
        if alien_x_random[i]<=0:

            alien_x_change[i] *= -1
            alien_y_random[i] += alien_y_change[i]


        elif alien_x_random[i]>= ancho:

            alien_x_change[i] *= -1
            alien_y_random[i] += alien_y_change[i]


        colision = is_collision(alien_x_random[i],alien_y_random[i],bullet_rect_x,bullet_rect_y)
        ...
        if colision == True:
            ...
            puntos += 1
            print(puntos)
            alien_x_random[i]= random.randint(0,largo)
            alien_y_random[i]= random.randint(50,150)

        pantalla.blit(alien[i],(alien_x_random[i],alien_y_random[i]))
